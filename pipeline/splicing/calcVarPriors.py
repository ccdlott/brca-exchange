...
def varInFirstThree(variant, donor=True):
    '''
...
def determineSpliceRescueSNS(variant, boundaries):
    ...
    if varCons == "stop_gained" and varInExon(variant) == True:
        ...
        if varInFirstThree(variant, donor=True) == True:
            priorProb = 0.97
            spliceRescue = 0
            frameshift = 0
        else:
            ...
            if inFrame == False:
                ...
                varWindowPos = getVarWindowPosition(variant, donor=True)
                inFirstThree = varInFirstThree(variant, donor=True)
